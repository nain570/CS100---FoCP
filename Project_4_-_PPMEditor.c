...
void flip(int ** destination, int rows, int cols, int max ){
	int i, j, tmp;
	int** dump = malloc(rows * sizeof(int *));
	for ( i = 0; i < rows; ++i)
		dump[i] = malloc(cols * sizeof(int));
	//storing in a temporary array
	for (i =  0; i < rows; i++){
		for (j = 0; j < cols; j++){ 
			dump[i][j] = destination[i][j];
		}
	}
	//flipping
	for (i= 0; i < rows; ++i){
		for (j = cols - 1 ; j >= 0; j-=3){
			destination[i][cols - j - 1] = dump[i][j-2];
			destination[i][cols - j] = dump[i][j-1];
			destination[i][cols - j + 1] = dump[i][j];
		}
	}
	free(dump);
}
...
void noise(int ** destination, int rows, int cols, int max ){
	...
	for (i =  0; i < rows; i++){
		for (j = 0; j < cols; j++){
			...
			choice = rand()%2;
			noise = rand()%max;
			if(choice == 0 && (destination[i][j] - noise) > 0)
				destination[i][j] -= noise;
			else if(choice == 1 && (destination[i][j] + noise) <=255)
				destination[i][j] += noise;
		}
	}
}
